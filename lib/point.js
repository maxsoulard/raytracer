...
Point.lengthBetween = function(p1, p2) {
    ...
    const lengthP1P2 = Math.sqrt(Math.pow(p1p2.x, 2) + Math.pow(p1p2.y, 2) + Math.pow(p1p2.z, 2));
    return lengthP1P2;
};
...
Point.angleBetween = function(a, b, c) {
    ...
    const k = (lengthABSquared + lengthACSquared - lengthBCSquared) / (2 * lengthAB * lengthAC);
