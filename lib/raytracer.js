...
module.exports = {
    ...
    _pixelFromIntersectedSphere: function(pixel, currentSphere, closest) {
        ...
        if (isLightInFrontOfSurface) {
            ...
            let inShadow = false;
            const shadowRay = {origin: closest, direction: lightDirection};
            const otherSpheres = this.spheres.filter(function(sphere) {
                return currentSphere != sphere;
            });
            
            let t = Infinity;
            otherSpheres.some(function(sphere) {
                let intersectionBetweenRayAndSphere = new Intersection(shadowRay, sphere).getClosestIntersect(t);
                //let intersectionBetweenRayAndCurrentSphere = new Intersection(shadowRay, currentSphere);
                const rayIntersectAnotherSphere = intersectionBetweenRayAndSphere.closest;
                t = intersectionBetweenRayAndSphere.t;
